...
ListSubjects = ['EG_I', 'HS', 'KJ_I', 'LJ', 'MG', 'MKL', 'SB', 'WS', 'AS', 'AP']
ListFreqs = [[8, 12], [13, 30], [31, 70], [71, 160]]
ListFilenames = ['respLocked_alpha_noBL', 'respLocked_beta_noBL', 'respLocked_lowGamma_noBL', 'respLocked_highGamma_noBL']
...
for subi, subject in enumerate(ListSubjects):
	for freqi, freq in enumerate(ListFreqs):
		...
		np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_time.npy', time, allow_pickle=True)
		np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_time_gen.npy', time_gen, allow_pickle=True)
		np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_y_pred.npy', y_pred, allow_pickle=True)
		np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_test_index.npy', test_index, allow_pickle=True)
		np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_score.npy', score, allow_pickle=True)

		if decCond is 'indItems':
			np.save(result_path + ListFilenames[0] + '/' + subject + '_WavDec_' + decCond + '_' + gen_filename + '_' + ListFilenames[0] + '_acc' + str(acc) + '_average_score.npy', average_score, allow_pickle=True)
